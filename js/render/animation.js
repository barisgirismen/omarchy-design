...
function tick(t) {
  ...
  grads.forEach(g => {
    ...
    g.setAttribute("x1", ax.x1.toFixed(2)); g.setAttribute("y1", ax.y1.toFixed(2));
    g.setAttribute("x2", ax.x2.toFixed(2)); g.setAttribute("y2", ax.y2.toFixed(2));
  });
  raf = requestAnimationFrame(tick);
}

export function hasMotion() { return state.mode === "holo"; }

export function syncAnim() {
  const wants = state.anim && hasMotion();
  if (wants && !raf) { t0 = 0; raf = requestAnimationFrame(tick); }
  if (!wants && raf) { cancelAnimationFrame(raf); raf = null; }
  const btn = $("btnPlay");
  if (btn) {
