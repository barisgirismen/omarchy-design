...
import { seg, row, slider, colorRow, checkbox, setTrack } from "./widgets.js";
...
export function buildFill() {
  ...
  if (state.mode === "linear") {
    box.appendChild(slider("angle", state.angle, 0, 360, 15, v => v + "°", v => { state.angle = v; state.preset = null; buildFill(); render(); }));
    const c = checkbox("snap band edges to the pixel grid", state.snap, v => { state.snap = v; buildFill(); render(); });
  ...
  if (state.mode === "holo") {
    ...
    const hue   = slider("base hue",   H.hue,    0, 360, 1,   v => Math.round(v) + "°",     v => { state.holo.hue = v;    state.preset = null; paintHoloTracks(); render(); }, { swatch: true });
    const sat   = slider("saturation", H.sat,   10, 100, 1,   v => Math.round(v) + "",      v => { state.holo.sat = v;    state.preset = null; paintHoloTracks(); render(); }, { swatch: true });
    const light = slider("lightness",  H.light, 35,  92, 1,   v => Math.round(v) + "",      v => { state.holo.light = v;  state.preset = null; paintHoloTracks(); render(); }, { swatch: true });
    box.append(hue, sat, light);
    box.appendChild(slider("bands",      H.cycles, .4,  5, .1,  v => v.toFixed(1),            v => { state.holo.cycles = v; state.preset = null; render(); }));
    box.appendChild(slider("sheen",      H.sheen,  0,   1, .01, v => Math.round(v * 100) + "", v => { state.holo.sheen = v;  state.preset = null; render(); }));
    box.appendChild(slider("angle",      state.angle, 0, 360, 5, v => v + "°",                v => { state.angle = v; state.preset = null; render(); }));
    ...
    function paintHoloTracks() {
      ...
      setTrack(sat,   `linear-gradient(90deg,hsl(${h} ${sat.range.min}% ${l}%),hsl(${h} 100% ${l}%))`);
      setTrack(light, `linear-gradient(90deg,hsl(${h} ${s}% ${light.range.min}%),hsl(${h} ${s}% ${light.range.max}%))`);
    }
    paintHoloTracks();
  }
...
export function buildCanvas() {
  ...
  b.replaceChildren();
  const bgSeg = div("seg");
  seg(bgSeg, [{ id: "none", label: "transparent" }, { id: "solid", label: "solid" }, { id: "gradient", label: "gradient" }],
    state.bg.mode, id => { state.bg.mode = id; buildCanvas(); render(); });
  b.appendChild(bgSeg);
  if (state.bg.mode !== "none") b.appendChild(colorRow("colour", state.bg.c1, v => { state.bg.c1 = v; render(); }));
  if (state.bg.mode === "gradient") {
    b.appendChild(colorRow("to", state.bg.c2, v => { state.bg.c2 = v; render(); }));
    b.appendChild(slider("angle", state.bg.angle, 0, 360, 15, v => v + "°", v => { state.bg.angle = v; render(); }));
  }
  b.appendChild(slider("padding", state.pad, 0, 24, .5, v => v + "u", v => { state.pad = v; render(); }));
