...
export function row(label, node, valNode) {
  ...
  d.append(l, node); if (valNode) d.append(valNode);
  return d;
}
...
export function slider(label, value, min, max, step, fmt, onInput, opts = {}) {
  ...
  r.oninput = () => { v.textContent = fmt(parseFloat(r.value)); onInput(parseFloat(r.value)); };
  const d = row(label, r, v);
  d.range = r;
  return d;
}

export function setTrack(sliderRow, css) { sliderRow.range.style.setProperty("--track", css); }

export function colorRow(label, value, onChange) {
  const wrap = div("cwrap");
  const c = input("color"); c.value = value;
  const t = input("text"); t.value = value;
  c.oninput = () => { t.value = c.value; onChange(c.value); };
  t.onchange = () => { if (isHex(t.value)) { c.value = t.value; onChange(t.value); } else t.value = c.value; };
  wrap.append(c, t);
...
export function checkbox(label, value, onChange) {
  ...
  const i = input("checkbox"); i.checked = value;
